...
<template>
  <div class="flex rounded-xl bg-zinc-50 px-2 py-6 sm:px-4 dark:bg-zinc-900">
    ...
    <div class="flex max-w-3xl items-center rounded-xl">
      <code v-if="debugMode" class="whitespace- -line">{{ message.content }}</code>
      <div
        v-else
        class="prose prose-base prose-zinc max-w-full prose-headings:font-semibold prose-h1:text-lg prose-h2:text-base prose-h3:text-base prose-p:first:mt-0 prose-a:text-blue-600 prose-code:text-sm prose-code:text-white prose-pre:p-2 dark:text-zinc-100"
      >
